...
var ASM_CONSTS = {
  86216: () => { return Module['suspended']; },  
 86248: ($0) => { Module['suspended'] = $0; },  
 86278: () => { var len = lengthBytesUTF8(tx) + 1; var s = _malloc(len); stringToUTF8(tx, s, len); return s; },  
 86375: () => { tx = ""; },  
 86388: ($0, $1) => { const classList = document.querySelector('#btn' + $0).classList; const highlight = 'highlight'; $1 ? classList.add(highlight) : classList.remove(highlight); },  
 86549: () => { return new Date().getTimezoneOffset() * 60 * 1000; },  
 86604: ($0) => { const date = new Date(Date.now() + $0); return date.getSeconds() | (date.getMinutes() << 6) | (date.getHours() << 12) | (date.getDate() << 17) | ((date.getMonth() + 1) << 22) | ((date.getFullYear() - 2020) << 26); },  
 86822: () => { document.getElementById("custom").style.display = ""; },  
 86878: () => { document.getElementById("classic").style.display = "none"; },  
 86939: () => { document.getElementById("classic").style.display = "none"; },  
 87000: () => { document.getElementById("custom").style.display = "none"; },  
 87060: ($0, $1) => { document.querySelectorAll("[data-com='" + $0 + "'][data-seg='" + $1 + "']") .forEach((e) => e.style.opacity = 1); },  
 87178: ($0, $1) => { document.querySelectorAll("[data-com='" + $0 + "'][data-seg='" + $1 + "']") .forEach((e) => e.style.opacity = 0); },  
 87296: () => { document.querySelectorAll("[data-com][data-seg]") .forEach((e) => e.style.opacity = 0); },  
 87388: () => { if (!Module['audioContext']) { Module['audioContext'] = new (window.AudioContext || window.webkitAudioContext)(); } },  
 87508: ($0) => { const audioContext = Module['audioContext']; if (!audioContext) return; if (!(audioContext._oscillator && audioContext._gain)) { const oscillator = audioContext.createOscillator(); const gain = audioContext.createGain(); oscillator.type = 'triangle'; oscillator.connect(gain); gain.connect(audioContext.destination); oscillator.start(0); audioContext._oscillator = oscillator; audioContext._gain = gain; } audioContext._oscillator.frequency.value = 1e6/$0; audioContext._gain.gain.value = volumeGain; },  
 88013: () => { const audioContext = Module['audioContext']; if (audioContext && audioContext._gain) { audioContext._gain.gain.value = 0; } },  
 88141: ($0, $1, $2) => { let filter = document.getElementById("ledcolor"); let color_matrix = filter.children[0].values.baseVal; color_matrix[0].value = $0 / 255; color_matrix[6].value = $1 / 255; color_matrix[12].value = $2 / 255; document.getElementById('light').style.opacity = Math.min(255, $0 + $1 + $2) / 255; },  
 88436: () => { return lat; },  
 88452: () => { return lon; },  
 88468: () => { return temp_c || 25.0; },  
 88495: () => { return -new Date().getTimezoneOffset(); },  
 88539: ($0) => { window.currentFaceName = UTF8ToString($0); }
};
